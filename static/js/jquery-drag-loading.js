 ...
 */
	var DragLoading = function (el,arrow,option){
	var defaults = {
		trigger:$('body'),
	...
	self.callback = {reload: params.onReload};
	...
	params.trigger.on({
		...
		touchmove: function (event){
			...
			if(arrow=='down' && (distanceY < 0 || elWindow.scrollTop() >10))return;
			if(arrow=='up' && (distanceY > 0 || elWindow.scrollTop() < $(document).height()-$(window).height()-70 ))return;
			...
			// 所有浏览器使用统一处理
			if ((elWindow.scrollTop() == 0 || elWindow.scrollTop() >= $(document).height()-$(window).height()-70 ) && distanceY > 0 ) {
				// other browser
			...
			if (data.markY > 0 && el.data('loading') != true) {
				// 太小不处理
				if(moveY < 0){valHeight = 0;}
				else{valHeight = Math.min(moveY, maxY);}

				// borderBottomWidth增加阻力
				var overflowHeight = 0 - valHeight / 2 + Math.max(0, moveY - maxY), borderBottomWidth = overflowHeight;
				if (overflowHeight > 0){borderBottomWidth = self.damping(overflowHeight);}
